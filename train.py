...
parser.add_argument('--load-model-InverseNet', help='optional model file to initialize with')
...
train_idx = idx[:255]
fixed_idx = idx[255:260]
eval_idx  = idx[260:275]
...
for epoch in range(args.initial_epoch, args.epochs):
    if epoch <args.lv1:
        train_level=1
        image_loss_func = vxm.losses.NCC_(win=3)
        
            
    elif epoch <args.lv2:
        train_level=2
        image_loss_func = vxm.losses.NCC_(win=5)

        if epoch ==args.lv1:
            for param in model.unet_model_4.parameters():
                param.requires_grad = False
            print(epoch,"model.unet_model_4.parameters().requires_grad = False")
        if epoch ==args.lv1+args.unfreeze_step:
            for param in model.unet_model_4.parameters():
                param.requires_grad = True
            print(epoch,"model.unet_model_4.parameters().requires_grad = True")

    else:
        train_level=3
        image_loss_func = vxm.losses.NCC_(win=7)
        if epoch ==args.lv2:
            for param in model.unet_model_4.parameters():
                param.requires_grad = False
            print(epoch,"model.unet_model_4.parameters().requires_grad = False")
            for param in model.unet_model_2.parameters():
                param.requires_grad = False
            print(epoch,"model.unet_model_2.parameters().requires_grad = False")
        if epoch ==args.lv2+args.unfreeze_step:
            for param in model.unet_model_4.parameters():
                param.requires_grad = True
            print(epoch,"model.unet_model_4.parameters().requires_grad = True")
            for param in model.unet_model_2.parameters():
                param.requires_grad = True
            print(epoch,"model.unet_model_2.parameters().requires_grad = True")
    # save model checkpoint
    if epoch % 20 == 0 and epoch!=args.initial_epoch:
    ...
    epoch_step_time = []
    inverse_epoch_loss = []
    inverse_epoch_total_loss = []
    inverse_epoch_step_time = []
    model.train()
    model_InverseNet.train()
    for step in range(args.steps_per_epoch):
        ...
        image_sim_loss = image_loss_func(warped_source, warped_target) * args.weight_image_sim
        smooth_loss = (smooth_loss_func(flow1,flow1)+smooth_loss_func(flow2,flow2)) * args.weight_smooth
        mid_dis_loss=mid_dis_loss_func(flow1,flow2)*args.weight_mid_dis
        
        loss=image_sim_loss+smooth_loss+mid_dis_loss
        
        loss_list.append(image_sim_loss.item())
        loss_list.append(smooth_loss.item())
        loss_list.append(mid_dis_loss.item())
        ...
        # InverseNet
        #-----------------------------------------------------
        inverse_step_start_time = time.time()
        f2m_flow=flow2.detach()
        if train_level==1:
            f2m_flow=model.fullsize_4(f2m_flow)
        elif train_level==2:
            f2m_flow=model.fullsize(f2m_flow)
        inverse_loss = 0
        inverse_loss_list = []
        y_identify,small_inverse_v,inverse_flow=model_InverseNet(f2m_flow)
        inverse_mse_loss=mse_threshold_loss_func(y_identify,torch.zeros_like(y_identify))*args.weight_inverse_mse
        ...
        inverse_loss_list.append(inverse_mse_loss.item())
        inverse_loss_list.append(inverse_smooth_loss.item())
        
        inverse_epoch_loss.append(inverse_loss_list)
        inverse_epoch_total_loss.append(inverse_loss.item())

        # backpropagate and optimize
        optimizer_InverseNet.zero_grad()
        inverse_loss.backward()
        optimizer_InverseNet.step()
        del flow1,flow2,source, target,warped_source, warped_target,y_identify,small_inverse_v,inverse_flow
        # get compute time
        inverse_epoch_step_time.append(time.time() - inverse_step_start_time)
    ...
    for i, f in enumerate(np.mean(epoch_loss, axis=0)):
        writer.add_scalar('losses_info_' + args.losses_names[i], f, epoch)
    writer.add_scalar('total_loss_info', np.mean(epoch_total_loss), epoch)

    # # print epoch info
    
    inverse_time_info = '%.4f sec' % np.mean(inverse_epoch_step_time)
    inverse_losses_info = ', '.join(['%.4e' % f for f in np.mean(inverse_epoch_loss, axis=0)])
    inverse_loss_info = 'Inverse_loss: %.4e  (%s)' % (np.mean(inverse_epoch_total_loss), inverse_losses_info)
    print(' - '.join((epoch_info, time_info, loss_info, inverse_time_info,inverse_loss_info)), flush=True)# inverse_time_info
    
    
# final model save
model.save(os.path.join(model_dir, '%04d.pt' % args.epochs))
model_InverseNet.save(os.path.join(model_dir, '%04d_InverseNet.pt' % args.epochs)) 
