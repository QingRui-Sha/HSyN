...
def save_kp_as_niigz(mask_array,node_x,affine,header,output_path):
        assert len(node_x.shape)==2 and node_x.shape[1]==3
        vertex_array = np.zeros_like(mask_array)  
        H,W,D=mask_array.shape
        # print(np.clip(node_x[:,0]+0.5,0,H),np.clip(node_x[:,1]),np.max(node_x[:,2]))
        vertex_array[(np.clip(node_x[:, 0]+0.5,0,H-1)).astype(np.int32), (np.clip(node_x[:, 1]+0.5,0,W-1)).astype(np.int32), (np.clip(node_x[:, 2]+0.5,0,D-1)).astype(np.int32)] = range(1,node_x.shape[0]+1)
        vertex_nii = nib.Nifti1Image(vertex_array, affine=affine, header=header)
        nib.save(vertex_nii, output_path)
def save_kp_as_csv(node_x,output_path):
        assert len(node_x.shape)==2 and node_x.shape[1]==3
        df = pd.DataFrame(node_x)#, columns=['x', 'y', 'z'])
